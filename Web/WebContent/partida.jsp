	...
	<div id="game" class="mainscreen">
		<div id="portraits" class="innersidebar" >
			<div id="p1" class="rival" >
				...
				<div>
					<p id="p1_name" class="player_name">Jugador1</p>
			...
			<div id="p2" class="rival" >
				<div class="portrait"></div>
				<div>
					<p id="p2_name" class="player_name">Jugador2</p>
			...
			<div id="p3" class="rival" >
				<div class="portrait"></div>
				<div>
					<p id="p3_name" class="player_name">Jugador3</p>
					<p id="p3_report" class="report">!</p>
				</div>
			</div>
			<div id="you" class="player" >
				<div class="playerportrait"></div>
				<p id="name" class="player_name"><%=(String)request.getAttribute("id_user")%> (Tú)</p>
			</div>
		</div>
		<div id="board" class="innermainscreen">
			...
			<div id="underhand">
				<button id="cancelHeist" onclick="cancelHeist()">Cancelar robo</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	const img = '<%=(String)request.getSession().getAttribute("foto")%>';
    ...
    if(user=="null"){
    	user = sessionStorage.getItem("id_user");
    	sessionStorage.removeItem("id_user");
    	name.innerHTML=user+'(You)';
    }
    //id del lobby
    var lobby = "<%=(String)request.getAttribute("id_lobby")%>";
    if(lobby=="null"){
    	lobby = sessionStorage.getItem("id_lobby");
    	sessionStorage.removeItem("id_lobby");
    }
    ...
    //Foto de perfil
    var foto = "<%=(String)request.getSession().getAttribute("foto")%>";
    ...
    socket.onopen = function (event) {
    	if(hand){
	    	var loginJSON = { msg_type: 'empezar', id_user:user , id_lobby:lobby , mazo:mazo , numuser:numuser , host:am_i_host , image:foto};
	    	socket.send(JSON.stringify(loginJSON));
    	} else {
    		var loginJSON = { msg_type: 'reconectar', id_user:user , id_lobby:lobby };
	    	socket.send(JSON.stringify(loginJSON));
    	}
    };
    ...
    function composite(msg){
    	...
    	//Comprobamos si nos toca
    	if(msg.turno==user){
    		my_turn=true;
    	}
		showMessage('Bienvenido de vuelta','sysmsg');
		showMessage('Es el turno de: '+msg.turno,'sysmsg');
    	//Comprobamos los reports
		var kicks=msg.kicks;
		for(var i=0 ; i<kicks.length ; i++){
	   		switch(kicks[i]){
		   		case p1name.innerHTML:
		   			p1_report.style.visibility='hidden';
		   			break;
		   		case p2name.innerHTML:
		   			p2_report.style.visibility='hidden';
		   			break;
		   		case p3name.innerHTML:
		   			p3_report.style.visibility='hidden';
		   			break;
	       	}
		}
		//Restauramos la mano
    ...
    function backHome() {
    	cleanExit=true;
    	var exitJSON = { msg_type: 'salir', id_user:user , id_lobby:lobby, numbombas:numBombas, portabilidad:false};
